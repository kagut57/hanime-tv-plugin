...
class HanimeTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def video_result(page):
            ...
            return {
                '_type': 'url',
                'url': urljoin('https://hanime.tv/videos/hentai/', video.get('slug')),
                ...
                'series': traverse_obj(page, ('hentai_franchise', 'title')),
            }

        page = self._download_json(
            'https://hanime.tv/api/v8/video',
            None,
            query={'id': video_id},
        )

        franchise = traverse_obj(page, ('hentai_franchise'), default={}, expected_type=dict)
        franchise_videos = traverse_obj(
            page,
        ...
        is_franchise = self._configuration_arg(
            'franchise',
            [False],
            ie_key=HanimeTVIE.ie_key(),
        )[0]
        ...
        if is_franchise:
            for entry in franchise_videos[1:]:
                page = self._download_json(
                    'https://hanime.tv/api/v8/video',
                    None,
                    query={'id': str_or_none(entry.get('slug'))},
                )
                entries.append(video_result(page))
...
class HanimeTVPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def fetch_page(pagenum):
            page = self._download_json(
                'https://hanime.tv/api/v8/playlist_hentai_videos',
                None,
            ...
            return [
                self.url_result(
                    urljoin('https://hanime.tv/videos/hentai/', e.get('slug')),
                    ie_key=HanimeTVIE.ie_key(),
                    video_id=e.get('slug'),
                    video_title=e.get('name'),
                )
                for e in data
            ]

        return self.playlist_result(
            OnDemandPagedList(fetch_page, num_page_ents),
            playlist_id,
        )
